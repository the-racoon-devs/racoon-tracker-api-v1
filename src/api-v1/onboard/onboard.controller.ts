...
import getOrganizations from "../organizations/organization.utils";
...
export default class OrganizationController {
  public registerOrganization = async (
    ...
    const { fullName, admins, isAdminVerified, organizationSlug } = req.body;
    try {
      const db = await dbPool.connect();
      try {
        await db.query("BEGIN");
        organization = await db.query(
          `INSERT INTO organizations( "fullName", "admins", "isAdminVerified", "organizationSlug", "createdAt") VALUES('${fullName}', '[${admins
            .map((adminId: string) => `"${adminId}"`)
            .join(
              ","
            )}]', '${isAdminVerified}', '${organizationSlug}',  NOW()) RETURNING *`
        );
